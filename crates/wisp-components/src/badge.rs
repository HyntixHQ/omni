...
pub struct BadgeProps<'a> {
    ...
    /// Optional Lucide icon rendered as a glyph before the text.
    /// Takes precedence over `icon` when both are set.
    pub lucide_icon: Option<Icon>,
}
...
pub fn badge(
    ...
    let colors = props
        .colors
        .clone()
        .unwrap_or_else(|| badge_variant_colors(props.variant));
    ...
    let icon_w = if props.icon.is_some() || props.lucide_icon.is_some() {
        icon_size + gap
    } else {
        0.0
    };
    ...
    if let Some(lucide) = props.lucide_icon {
        let icon_y = y + (bh - icon_size) / 2.0;
        wisp::lucide::draw_lucide_icon(pixmap, lucide, content_x, icon_y, icon_size, colors.fg);
